...
plot(0,type='n', #make empty plot 
     ...
     main = '32°C Survival Over Time', # main title (on middle of plot)
...
AirSol32 <- points(summary2$time[6:10], summary2$prop.surv[6:10], type = 's', col = ColorAirSol[2], lwd = 3)
...
#check
WaterTP32 <- points(summary2$time[66:70], summary2$prop.surv[66:70], type = 's', col = ColorWaterTP[2], lwd = 3)
#
...
AirTP32 <- points(summary2$time[26:30]+0.2, summary2$prop.surv[26:30], type = 's', col = ColorAirTP[2], lwd = 3)
WaterTP32 <- points(summary2$time[66:70], summary2$prop.surv[66:70]+0.01, type = 's', col = ColorWaterTP[2], lwd = 3)
WaterSol32 <- points(summary2$time[46:50]+0.2, summary2$prop.surv[46:50], type = 's', col = ColorWaterSol[2], lwd = 3)
AirSol32 <- points(summary2$time[6:10], summary2$prop.surv[6:10], type = 's', col = ColorAirSol[2], lwd = 3)
